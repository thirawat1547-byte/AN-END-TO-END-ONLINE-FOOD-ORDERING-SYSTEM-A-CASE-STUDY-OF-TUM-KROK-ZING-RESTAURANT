...
function openEditModal(menu) {
  ...
  form.value = { 
    ...menu, 
    allergen_ids: Array.isArray(menu.allergen_ids) ? [...menu.allergen_ids] : [] 
  }
  isModalOpen.value = true
}
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
      <div class="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4 my-8 max-h-[90vh] overflow-y-auto">
        ...
        <div class="space-y-3.5 text-xs">
          ...
          <!-- Allergens Multi-select (Allergens Warning) -->
          <div>
            <label class="block font-bold text-slate-700 mb-1.5 flex items-center justify-between">
              <span>สารก่อภูมิแพ้ (Allergens Warning)</span>
              <span class="text-[10px] text-amber-700 font-semibold bg-amber-50 px-2 py-0.5 rounded border border-amber-200">⚡ ซิงก์แสดงผล Real-time ทันที</span>
            </label>
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-2 p-3 bg-amber-50/40 rounded-xl border border-amber-200/80">
              <label 
                v-for="al in adminStore.allergens" 
                :key="al.allergen_id"
                :class="[
                  'flex items-center gap-2 p-2 rounded-lg border cursor-pointer text-[11px] transition select-none',
                  form.allergen_ids && form.allergen_ids.includes(al.allergen_id)
              ...
              >
                <input 
                  ...
                  class="rounded text-[#2d5a43] focus:ring-[#2d5a43] w-3.5 h-3.5"
                />
                <span class="text-sm shrink-0">{{ al.icon }}</span>
                <span class="truncate">{{ al.allergen_name }}</span>
              </label>
            </div>
            <p class="text-[10px] text-slate-500 mt-1">
              * รายการที่เลือกจะมีเขียนเตือนอยู่ตรงหลังชื่อเมนูเมื่อลูกค้ากดเลือกเมนูนั้นในหน้าสั่งออนไลน์และภายในร้าน
            </p>
          </div>
        </div>
