<template>
  <div class="cart-item">
    <div class="item-info">
      ...
      <div class="customizations" v-if="(item.spicyLevel && item.spicyLevel !== 'normal') || item.specialInstructions || (item.addons && item.addons.length > 0)">
        <span class="spicy-level" v-if="item.spicyLevel && item.spicyLevel !== 'normal'">
          🌶️ {{ getSpicyLabel(item.spicyLevel) }}
        </span>
        <span class="addon-tag" v-for="addon in item.addons" :key="addon.name">
